Scriptname ProvDiag:ProvDiag extends HAL7032:HAL7032

; Provisioner Diagnostics v1.0
; Fully functional example application for HAL7032 Terminal Emulation
; custom scripting by niston

; External Dependencies (other than F4SE):
; - F4SE
; - HAL7032 Terminal Emulation
; - RenameAnything
; - Vault DLC
...
Event OnHAL7032Initialize(ObjectReference refTerminal)
    ; ignored
EndEvent

Event OnHAL7032Ready()
    ...
    ; navigate to provisioner list
    NaviProvisionerListFirst()
EndEvent

Event OnHAL7032Shutdown()
    ; log & cleanup
    Debug.Trace(Self + ": DEBUG - OnHAL7032Shutdown event received.")
...
Int Function PrintProvisionerListEntries(Int startIndex)
    ...
    If (bAsyncKeyRead)
        ...
        PlaySound("TRPTriggerBeepFireLPM")
...
Event OnHAL7032ReadAsyncCompleted(String readBuffer)
...
Function MenuProvisionerDetails(ObjectReference refProvisioner)
    ...
    PrintLine("Options: ")
    PrintLine("  [R] : Reset AI             [A] : Cycle Alias         [S] : Summon")                
    PrintLine("  [O] : Show OMods           [I] : Inventory           [K] : Kill")
    PrintLine("  [U] : Update Display       [T] : Trade               [N] : Rename")
    PrintLine("  [P] : Set Protected        [E] : Set Essential       [M] : Set Mortal")
    ...
    ElseIf (selectedOption == "p")
        PrintLine("Setting protected flag...")
        (refProvisioner as Actor).SetProtected(true)
        Sleep(1.0)
        NaviProvisionerDetails(refProvisioner) 
    ElseIf (selectedOption == "e")
        PrintLine("Setting essential flag...")
        (refProvisioner as Actor).SetEssential(true)
        Sleep(1.0)
        NaviProvisionerDetails(refProvisioner)
    ElseIf (selectedOption == "m")
        PrintLine("Clearing protected & essential flags...")
        (refProvisioner as Actor).SetEssential(false)
        (refProvisioner as Actor).SetProtected(false)
        Sleep(1.0)
        NaviProvisionerDetails(refProvisioner)
    ...
    ElseIf (selectedOption == "k")
        Print("? Kill Provisioner - Are you sure about this? (Y/N)")
        If (ReadKey() == "y")
            PrintLine("Y")
            ...
            EndIf
            ; make sure their supply line is unlinked, so no "ghost" line remains
            RemoveSupplyLine(lctRouteOrigin, lctRouteDest)
            ; kill the provisioner dead, even if essential/protected
...
String Function GetActorFlags(Actor actActor)
    ...
    EndIf
    If (actActor.IsProtected())
        flagsString = AppendWith(flagsString, "IsProtected")
    EndIf
...
String Function GetLocationDescription(ObjectReference refProvisioner)
    ...
    If (refProvisioner.GetParentCell() != none && refProvisioner.GetParentCell().IsInterior())
        ; in interior
        If (locName == "")
            Return "INTERIOR"
        Else
            Return locName + " (INTERIOR)"
        EndIf
    Else
        ; in exterior
        If (locName == wsName || locName == "")
            Return wsName
        Else
...
Bool Function RemoveSupplyLine(Location lctOrigin, Location lctDestination)
    Bool locationsLinked = lctOrigin.IsLinkedLocation(lctDestination, kwdWorkshopLinkCaravan)
    If (locationsLinked)
        lctOrigin.RemoveLinkedLocation(lctDestination, kwdWorkshopLinkCaravan)
        lctDestination.RemoveLinkedLocation(lctOrigin, kwdWorkshopLinkCaravan)        
        Return True
    Else        
        Return False
    EndIf
